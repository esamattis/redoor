...
const AGENT_NAME = "reload-config-agent";
const SECOND_AGENT_NAME = "reload-config-agent-two";
...
beforeAll(async () => {
    ...
    agentDir = tempFiles.tempDirectory({ suffix: "-reload-agent-cwd" });
    secondAgentDir = tempFiles.tempDirectory({
        suffix: "-reload-agent-two-cwd",
    });
...
describe("POST /api/v1/config/reload", () => {
    it("rejects invalid config without restarting the process", async () => {
        ...
        await expect(apiClient.reloadConfig()).rejects.toSatisfy(
            (error: unknown) => {
    ...
    it("restarts in place and applies a mutated config.toml", async () => {
        ...
        await waitForValue({
            timeoutMs: 30000,
            description: "server to come back with both agents after reload",
            predicate: async () => {
                try {
                    const agents = await apiClient.listAgents();
