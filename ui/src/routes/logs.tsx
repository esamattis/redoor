...
import type { ServerLogEvent } from "../api-client";
...
function parseServerLogEvent(data: string): ServerLogEvent {
...
/** Owns the log socket only while this route is mounted so background viewing has no hidden cost. */
function ServerLogsPage() {
    ...
    React.useEffect(() => {
        ...
        /** Allocates stable local keys because identical logger text is valid and must render twice. */
        ...
        /** Schedules at most one reconnect so repeated close/error events cannot multiply sockets. */
        ...
        /** Opens a fresh socket whose first snapshot replaces entries missed during any disconnect. */
        const connect = () => {
            if (!active) {
                return;
            }
            ...
            const nextSocket = new WebSocket(api.getServerLogsWebSocketUrl());
            socket = nextSocket;
            ...
            nextSocket.addEventListener("message", (event) => {
                ...
                try {
                    const message = parseServerLogEvent(event.data);
                    switch (message.type) {
                        case "snapshot":
                            setEntries(
                                message.entries
        ...
        <div className="flex h-full min-h-0 flex-col p-8">
            <div className="mx-auto flex h-full min-h-0 w-full max-w-7xl flex-col">
                ...
                <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-400">
                    <span
                        role="status"
                        aria-label="Server log connection status"
                    >
                        {connectionLabel}
                    </span>
                    {statusMessage ? (
