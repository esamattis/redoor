...
fn default_config_content(username: &str, password: &str, agent_token: &str) -> String {
...
pub(crate) async fn create_default_config_if_missing(
    ...
    file.write_all(default_config_content(&username, &password, &agent_token).as_bytes())
        .await
...
"#;
    ...
    async fn test_create_default_config_if_missing() {
        ...
        assert_eq!(
            config.server.password, bootstrap.password,
        ...
        assert!(
            bootstrap.password.len() >= 32 && bootstrap.agent_token.len() >= 32,
            "bootstrap secrets must be high entropy"
        );
        assert_ne!(
            bootstrap.password, bootstrap.agent_token,
