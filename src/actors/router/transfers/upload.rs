...
pub(crate) fn route_chunk(
    ...
    if let Some(agent_info) = state.agents.by_id.get(&request.agent_id).cloned() {
        ...
        tokio::spawn(async move {
            ...
            let route = FinishUploadChunkRoute {
                agent_id,
                request_id,
                bytes,
                is_error,
            ...
            if let Err(error) = myself.send(RouterMsg::FinishRoutedUploadChunk(route)).await {
                if let RouterMsg::FinishRoutedUploadChunk(route) = error.0 {
