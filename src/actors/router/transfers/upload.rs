...
pub(crate) fn start(router: &mut Router, request: StartUploadRequest) {
    ...
    if let Some(agent_connection) = router.agents.by_id.get(&request.agent_id).cloned() {
...
pub(crate) fn route_chunk(
    ...
    let transfer = match router.streams.uploads.get(&request.request_id) {
    ...
    if let Some(agent_connection) = router.agents.by_id.get(&request.agent_id).cloned() {
        ...
        if is_error {
            ...
            progress::mark_transfer_errored(router, request_id.as_transfer_id(), error_message);
        }
...
pub(crate) fn finish_routed_chunk(
    ...
    match router.streams.uploads.get(&route.request_id) {
        Some(transfer) => {
    ...
    if !route.send_succeeded {
        let should_cancel_agent = match router.streams.uploads.get_mut(&route.request_id) {
            Some(transfer) => {
                if transfer.agent_id != route.agent_id {
                    return Err(RouterError::StreamNotFound {
                        agent_id: route.agent_id.to_string(),
        ...
        if should_cancel_agent
            && let Some(agent_connection) = router.agents.by_id.get(&route.agent_id)
        {
            log!(
                Level::Info,
    ...
    if !route.is_error {
        progress::increment_bytes(router, route.request_id.as_transfer_id(), route.bytes);
    }
...
pub(crate) fn finish_transfer(
    ...
    let transfer_state = match router.streams.uploads.get_mut(&request_id) {
        Some(transfer) => {
        ...
        None => {
            if router.streams.downloads.contains_key(&request_id) {
                log!(
                    Level::Warning,
    ...
    let completion_result = match &result {
        CommandResult::RawUpload => {
            progress::mark_transfer_completed(router, request_id.as_transfer_id());
            log!(
                Level::Info,
        ...
        CommandResult::Error { message, .. } => {
            progress::mark_transfer_errored(router, request_id.as_transfer_id(), message.clone());
            log!(
                Level::Info,
